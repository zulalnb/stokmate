...
- **Auth state in a plain module, not Context/Zustand** — the Axios interceptor and the `_authenticated` guard are not React components and need to read/write the token synchronously; Context alone does not provide this. No screen reactively re-renders on token change (login and logout both navigate instead), so no subscription mechanism — not `useSyncExternalStore`, not Zustand's `useStore` — is needed either; a plain module-level cache is sufficient.
...
- The same HTTP/service architecture as the web client was intentionally carried over: Axios + interceptors + layered `client → services → hooks` separation. Session state is deliberately different from web, though: mobile screens do need to reactively re-render on session change, so `src/lib/session-store.ts` is a `subscribe`/`getSnapshot` store built for `useSyncExternalStore` — unlike web's plain, non-reactive module.
