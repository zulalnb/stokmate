...
Most examples on the internet and the shadcn data-table docs show the old API (`useReactTable`, `flexRender`, `getCoreRowModel`). Mixing the two leads to confusing type errors. This project has TanStack intent skills configured (see `AGENTS.md` § Skill Loading): run `pnpm dlx @tanstack/intent@latest list` from `web/` and load the matching `@tanstack/table-core` / `@tanstack/react-table` skill (`#core`, `#migrate-v8-to-v9`, `#api-not-found`, etc.) instead of guessing from a v8-shaped example or reading `node_modules` types cold.

**Known pitfall (server-side pagination):** The shadcn data-table docs assume client-side pagination. In this project, pagination and sorting are on the API side; do **not** add `getPaginationRowModel()` and `getSortedRowModel()`. Both `rowSortingFeature` (`manualSorting: true`) and `rowPaginationFeature` (`rowCount`-driven, no `onPaginationChange`) are registered for state/read access only — `pageCount` comes from `table.getPageCount()`, not a manual `Math.ceil` (see AGENTS.md § Tables for the full rationale, including why pagination has no change handler while sorting does).

**Known pitfall (column width):** With the default `table-auto` layout, column widths are measured by content; as you switch pages, long product names can shift other columns. This project does **not** use `table-fixed` (an earlier setup attempt did — since reversed): widths are applied selectively via the column definition's `meta.className` (`max-w-*`/`w-*`) only on columns that need it, not on every column (see AGENTS.md § Tables).

**Verification:** Switch between two different pages; column widths should stay stable for columns that set `meta.className`. In the Network tab, there should be only one `GET /products` request per page change.
