...
Column definitions are centralized in a single file, cell rendering is typed, and the table structure is separated from the screen component. This is a pattern I've used before, so setup and debugging time was short. The table code fits a fixed template: `data-table-features.tsx` (tableFeatures config), `columns.tsx` (columns), `data-table-column-header.tsx` (sortable header), `data-table.tsx` (`useTable()` + render), `data-table-pagination.tsx` (pagination footer) — for a new feature table, the same structure is repeated. (Originally 4 files with a plain `sortable-header.tsx`; see the later "Pagination as a real table feature" entry below for why pagination joined the template as a registered table feature too.)

**Cost:** Since filtering is handled entirely outside the table (API-side, via URL search params), the library's data processing layer is not used for it (`filteredRowModel` is not added, no `columnFilteringFeature`). Sorting and pagination are registered as table features (`rowSortingFeature`, `rowPaginationFeature`) for state/column/row API only — `sortedRowModel`/`paginatedRowModel` are never added, so nothing is re-sorted or re-paginated client-side; both stay server-side. Also, the project uses the library's new API (`tableFeatures`, `useTable`); most online examples show the old API — this project has TanStack intent skills configured for exactly this problem (see `AGENTS.md` § Skill Loading) instead of relying on manual node_modules spelunking.
...
## Pagination as a real table feature — read-only, no change handler

**Selected:** Register `rowPaginationFeature` in `data-table-features.tsx` (no `paginatedRowModel`, same reasoning as `rowSortingFeature`), feed it `state: { pagination }` + `rowCount` from `data-table.tsx`, and move rendering of `DataTablePagination` from the route into `DataTable` (passing it the `table` instance) so it reads `page`/`pageCount`/`getCanPreviousPage`/`getCanNextPage` from the table instead of recomputing `Math.ceil(total / pageSize)` independently. Deliberately **no** `onPaginationChange` and no `manualPagination` flag.
**Rejected:** (a) Keep pagination fully external to the table, as it was — a standalone component computing its own page count from raw `page`/`total`/`pageSize` props. (b) Wire up `onPaginationChange` + `manualPagination: true` to mirror sorting exactly.

Sorting and pagination looked symmetric at first (`manualSorting`/`rowSortingFeature` vs. a hypothetical `manualPagination`/`rowPaginationFeature`), but they aren't: sorting's interaction genuinely goes through the table's own column API (`data-table-column-header.tsx`'s click handler is `column.getToggleSortingHandler()`), so it needs `onSortingChange` to do anything. Pagination's interaction is 100% `<Link search={...}>`-driven and never calls `table.nextPage()`/`setPageIndex()`, so `onPaginationChange` would never have a live caller. Read `@tanstack/table-core`'s `setStateSlice` source confirms this is safe either way: `if (!onChange) return` — without `onPaginationChange`, any internal attempt to mutate pagination state (including the auto-reset-on-data-change path) is already a complete no-op, so omitting `manualPagination: true` isn't a regression either. The table stays a correct, read-only single source of truth for `page`/`pageCount`/`canNext`/`canPrev` without carrying dead plumbing.

**Cost:** The route no longer renders `<DataTablePagination>` directly; `DataTable` now owns that composition. Pagination navigation is still `<Link search={...}>`-driven, so hover-preload and normal anchor semantics are unchanged. If a future feature needs the table to imperatively change pages itself (not just reflect the URL), `onPaginationChange` + `manualPagination: true` would need to be added back then.
